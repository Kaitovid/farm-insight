...
import { Plus, Search, TrendingUp, TrendingDown, Trash2, ArrowLeft } from 'lucide-react';
...
import { useAviculturaMovimientos, useCreateMovimiento, useDeleteMovimiento, Sector } from '@/hooks/useAvicultura';
...
export default function SectorPage({ sector, title, description }: SectorPageProps) {
  const navigate = useNavigate();
  const { data: movimientos = [], isLoading } = useAviculturaMovimientos(sector);
  const { data: categorias = [] } = useCategorias();
  const createMovimiento = useCreateMovimiento(sector);
  const deleteMovimiento = useDeleteMovimiento(sector);
  ...
  const totalVentas = movimientos.filter(m => m.tipo === 'venta').reduce((acc, m) => acc + Number(m.monto || 0), 0);
  const totalGastos = movimientos.filter(m => m.tipo === 'gasto').reduce((acc, m) => acc + Number(m.monto || 0), 0);
    ...
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" onClick={() => navigate('/avicultura')} className="h-9 w-9 shrink-0">
            ...
            <DialogContent className="w-[95vw] max-w-[425px] max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                ...
                <DialogDescription className="text-sm">
                  Agrega una nueva venta o gasto al sector {title}.
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                  ...
                  >
                    ...
                    <SelectContent>
                      {nuevoMovimiento.tipo === 'venta'
                        ? categorias
                            .filter(c => c.sector.toLowerCase().includes('venta'))
                            .map(c => <SelectItem key={c.id} value={c.nombre}>{c.nombre}</SelectItem>)
                        : categorias
                            .filter(c => c.sector.toLowerCase().includes('gasto'))
                            .map(c => <SelectItem key={c.id} value={c.nombre}>{c.nombre}</SelectItem>)
                      }
                    </SelectContent>
                  </Select>
                ...
                {/* Cantidad pollos (solo si aplica) */}
                {sector === 'avicola' && (
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setNuevoMovimiento({ ...nuevoMovimiento, cantidadPollos: e.target.value })}
                    />
                  </div>
                )}
                {/* Monto */}
        ...
        <div className="rounded-xl border border-border bg-card p-4 sm:p-5 shadow-sm animate-fade-up opacity-0" style={{ animationDelay: '300ms', animationFillMode: 'forwards' }}>
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className={`text-lg sm:text-xl font-bold truncate ${utilidad >= 0 ? 'text-farm-green' : 'text-destructive'}`}>
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden animate-fade-up opacity-0" style={{ animationDelay: '400ms', animationFillMode: 'forwards' }}>
            ...
            <tbody>
              {movimientosFiltrados.map((m) => (
                <tr key={m.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                  <td className="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-foreground whitespace-nowrap">
                    {formatDate(new Date(m.fecha))}
                  </td>
                  <td className="px-3 sm:px-4 py-2 sm:py-3">
                    <span className={`inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap ${m.tipo === 'venta' ? 'bg-farm-green/10 text-farm-green' : 'bg-farm-orange/10 text-farm-orange'}`}>
        ...
        {movimientosFiltrados.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">
            <p className="text-sm">No se encontraron movimientos en el sector {title}</p>
          </div>
        )}
      </div>
    </div>
