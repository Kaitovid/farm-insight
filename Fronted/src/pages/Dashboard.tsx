...
export default function Dashboard() {
  ...
  const metricas = useMemo(() => {
    const ventasTotales = movimientos.filter(m => m.tipo === 'venta').reduce((acc, m) => acc + Number(m.monto || 0), 0);
    const gastosTotales = movimientos.filter(m => m.tipo === 'gasto').reduce((acc, m) => acc + Number(m.monto || 0), 0);
    ...
    const totalPollos = movimientos.reduce(
      (acc, m) => acc + Number(m.numero_pollos || 0),
      0
    );
  ...
  const chartData = useMemo(() => {
    ...
    const movimientosPorMes = movimientos.reduce((acc, m) => {
      ...
      if (m.tipo === 'venta') {
        acc[mesKey].ventas += Number(m.monto || 0);
      } else if (m.tipo === 'gasto') {
        acc[mesKey].gastos += Number(m.monto || 0);
      }
    ...
    const gastosPorCategoria = movimientos
      ...
      .reduce((acc, m) => {
        ...
        acc[categoria] = (acc[categoria] || 0) + Number(m.monto || 0);
        return acc;
